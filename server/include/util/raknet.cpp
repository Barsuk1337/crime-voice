...
void RakNet::Process() {
	...
	// Rpc's sending...
	{
		const std::unique_lock<std::mutex> lock(RakNet::rpcQueueMutex);
	...
	// Packets sending...
	{
		const std::unique_lock<std::mutex> lock(RakNet::packetQueueMutex);
	...
	// Kicking players...
	{
		const std::unique_lock<std::mutex> lock(RakNet::kickQueueMutex);
